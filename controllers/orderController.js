...
// ── POST /api/orders ──────────────────────────────────────────────────────────
// Creates a PENDING order. Stock is deducted only after payment is verified.
export const createOrder = async (req, res, next) => {
  try {
    ...
    const order = await Order.create({
      ...
      shippingCharge,
      totalAmount,
      // For COD, mark as pending — for Razorpay, verifyRazorpayPayment will mark paid + deduct stock
      status: paymentMethod === "cod" ? "Processing" : "Pending",
      isPaid: paymentMethod === "cod", // COD is effectively "paid on delivery"
    });
